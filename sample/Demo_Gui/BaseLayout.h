...
namespace wraps
{
	...
	protected:
		...
		template <typename T>
		void AssignWidget(T * & widget, const std::string& name, bool _throw = true)
		{
			widget = nullptr;
			...
			ASSERT( ! _throw, "widget curName '" << name << "' in layout '" << mLayoutName << "' not found.");
		}

		template <typename T>
		void AssignBase(T * & widget, const std::string& name, bool _throw = true)
		{
			widget = nullptr;
			for (GUI::VectorWidgetPtr::iterator iter=mListWindowRoot.begin(); iter!=mListWindowRoot.end(); ++iter)
			{
