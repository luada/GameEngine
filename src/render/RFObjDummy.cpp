...
CRFObj_Dummy::~CRFObj_Dummy()
{
	if(m_pSceneNode)
	{
		if ( m_pEffect != NULL )
		{
			MM::EffectManager::getSingleton().removeEffect(m_pEffect);
			m_pEffect = NULL;
		}
...
VOID CRFObj_Dummy::Attach_ProjTexture(PROJTEX_TYPE type, BOOL bShow, FLOAT Ring_Range, FLOAT fHeight)
{
	...
	if(Ring_Range != 100.0f && m_pEffect)
	{
		...
		{
			MM::EffectElement *eff_element = m_pEffect->getElement(i);

			if (eff_element->getType() == "Projector")
			{
				static_cast<MM::ProjectorElement*>(eff_element)->setProjectionSize(Ring_Range);
			}
		}
	}
...
VOID CRFObj_Dummy::Attach_Effect(LPCTSTR szEffectName)
{
	...
	//创建新的特效
	m_pEffect = MM::EffectManager::getSingleton().createEffect(szEffectName);
	if ( m_pEffect != NULL )
	{
		//挂接到场景节点
