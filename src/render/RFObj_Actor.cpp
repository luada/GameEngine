...
#include "ccom/TDDBC_Struct.h"
#include "ccom/TDDataBase.h"
#include "ccom/TDException.h"
...
VOID CRFObj_Actor::Actor_SetFile(LPCTSTR szActorFile)
{
	...
	m_pWXObject->setProperty(FOBJ_ACTOR_FILE, MM::String(szActorFile));

//	AxTrace(0, 0, "%s", szActorFile);
	//检测资源Load情况
	mModel = ((MM::LogicModelObject*)m_pWXObject.get())->getLogicModel();
...
VOID CRFObj_Actor::Actor_SetEffectExtraTransformInfos(UINT uEffect, fVector3& fvPosition)
{
	...
	MM::Effect *pEffect = mModel->getEffect(uEffect);
	if(pEffect != NULL)
	{
		MM::TransformInfos tempTransformInfos;
		fVector3 fvGfxPos;
		CRenderSystem::GetMe()->Axis_Trans(CRenderSystem::AX_GAME, fvPosition, CRenderSystem::AX_GFX, fvGfxPos);
		MM::TransformInfo tempTransformInfo(Ogre::Vector3(fvGfxPos.x, fvGfxPos.y, fvGfxPos.z));
    ...
    case SELECT_RING:
        {
			if (show)
			{
				mModel->addProjector(MM::MPT_SELECTED, "selected_projector", show, Ring_Range);
			}else
			{
				mModel->removeProjector(MM::MPT_SELECTED, "selected_projector");
			}
            break;
        }
    case SHADOW_BLOB:
        {
            mModel->addProjector(MM::MPT_SHADOW, "shadow_projector", show, Ring_Range);
            break;
        }
    }
}
...
void CRFObj_Actor::Attach_Object(tEntityNode *pObject, LPCTSTR szAttachLocator)
{
	if ( mModel != NULL && pObject!= NULL && pObject->GetType() == ETYPE_ACTOR && szAttachLocator != NULL )
	{
		...
		MM::LogicModel* pActorImpl = pRFObject->GetActorImpl();
		if ( pActorImpl != NULL )
		{
...
VOID CRFObj_Actor::Actor_SetUIVisibleFlag(VOID)
{
	...
	mModel->setVisibleFlag((unsigned int)MM::OVF_GUI_ELEMENTS);
...
//		mDObject->changeAttribute("RightWeaponObj", "大刀13.obj");
		m_leftWeaponModel = MM::VariantCast<MM::LogicModel*>(mModel->getAttribute("LeftWeaponObj"));
	}
    ...
    if (m_leftWeaponModel)
    {
		...
		{
			m_leftWeaponModel->delAllEffect();
			m_leftWeaponEffect = m_leftWeaponModel->addEffect(szEffectName,"武器特效点",MM::LogicModel::GTIT_ALL);
			MM::Effect* effect = m_leftWeaponModel->getEffect(m_leftWeaponEffect);
...
VOID CRFObj_Actor::SetRightWeaponEffect(LPCTSTR szEffectName, UINT color)
{
	...
	{
//		mDObject->changeAttribute("RightWeaponObj", "大刀13.obj");
		m_rightWeaponModel = MM::VariantCast<MM::LogicModel*>(mModel->getAttribute("RightWeaponObj"));
	}
    ...
    if (m_rightWeaponModel)
    {
		...
		{
			m_rightWeaponModel->delAllEffect();
			m_rightWeaponEffect = m_rightWeaponModel->addEffect(szEffectName,"武器特效点",MM::LogicModel::GTIT_ALL);
			MM::Effect* effect = m_rightWeaponModel->getEffect(m_rightWeaponEffect);
