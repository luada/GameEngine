...
#include "moo/TerrainData.h"
#include "moo/FrameStatsListener.h"
#include "moo/ObjectProxy.h"
...
#include "moo/SceneInfo.h"
#include "moo/Terrain.h"
#include "moo/ResourceSolverHelper.h"
#include "moo/PostFilters.h"
#include "moo/PostFilterManager.h"
#include "ResourceLoadListener.h"
#include "moo/ExpatParser.h"
#include "ccom/TDVariable.h"
#include "TerrainGrid.h"
#include "ccom/TDKernel.h"
#include <OgreParticleSystemManager.h>
#include <OgreParticleSystem.h>
#include "ccom/TDProfile.h"
#include "ccom/TDSound.h"
#include "moo/LogicModel.h"
#include "RFFakeObjectManager.h"
#include "moo/SceneInfo.h"
#include "moo/EnviromentObject.h"
#include <OgreMeshManager.h>
#include <OgreSkeletonManager.h>
#include <OgreMaterialManager.h>
#include <OgreSceneManager.h>
#include <OgreStringConverter.h>
#include "ccom/TDWorldSystem.h"
#include "ccom/TDEventDefine.h"
#include "moo/LogicModelObject.h"
#include "ccom/TDTimeSystem.h"
#include "ResourceProvider.h"
#include "moo/LogicModelManager.h"
...
{
	register MM::TerrainData* pTerrainData = m_pRenderSystem->getTerrainData();
	//尚未加载
	...
	case AX_SCREEN:
		{
			...
			BOOL bRet = m_pRenderSystem->getTerrainIntersects(MM::Point((INT)fvSource.x, fvSource.y), vRenderPos);
...
//	std::locale::global(langLocale);
	...
	{
		//传入渲染窗口句柄
		MM::System::NameValuePairList ViewSetting;
		...
		m_pRenderSystem->setup((MM::ulong)(UINT_PTR)g_hMainWnd, &ViewSetting);
		...
		//注册所有全局渲染器
		MM::registerAllPostFilterFactories(m_pPostFilterManager);
...
//		m_pPostFilterManager->createPostFilter("HeatVision", m_pRenderSystem->getViewport());
		...
		//--------------------------------------------
		//设置特效系统的声音播放函数
		MM::LogicModel::SetPlaySoundFuncton((OnPlaySound)(g_pSoundSystem->GetPlaySoundFunc()));
		MM::LogicModel::SetStopSoundFunction((OnStopSound)(g_pSoundSystem->GetStopSoundFunc()));
		MM::LogicModel::SetGetHeightInWorldListener((GetHeightInWorld)(g_pWorldSystem->GetTerrainHeightFunc()));

		// 初始化 fake obeject manager
		m_pFakeObjectManager = new CFakeObjectManager;
		m_pFakeObjectManager->Initial();

//		MM::Effect::setDefaultIterativeInterval(0.01f);
                ...
                if (Ogre::StringUtil::endsWith(listener._scriptName, ".material", TRUE))
                {
                    ...
                    if (sscanf(e.getDescription().c_str(), "Resource with the name %255s already exists.", materialName) == 1)
                    {
                        ...
                        if (!material.isNull())
                        {
                            ...
                            Ogre::StringVectorPtr currentNames = MM::findResourceFilenames(listener._groupName, listener._scriptName);
                            ...
                            {
                                Ogre::StringVectorPtr originNames = MM::findResourceFilenames(material->getGroup(), material->getOrigin());
...
VOID CRenderSystem::Scene_PreLoad(const STRING& theScenePreload)
{
	...
	if(dwSize > 0)
	{
		...
		do
		{
			...
			MM::LogicModelManager::getSingleton().loadModelResource(szLine);
		}while(true);
...
{
	MM::ExpatParser parserExpat(NULL);
	...
	//设置LightMap
	m_pRenderSystem->getTerrain()->setLightmapQuality(MM::Terrain::LMQ_LOW);
	m_pRenderSystem->getTerrain()->buildGeometry(m_pRenderSystem->getBaseSceneNode());

	//取得环境特效
	MM::SceneInfo::ObjectsByTypeRange objEnviroment = 
		m_pRenderSystem->getSceneInfo()->findObjectsByType("Enviroment");

	if (objEnviroment.first != objEnviroment.second)
	{
		m_pEnviromentObject = static_cast< MM::EnviromentObject * >(objEnviroment.first->get());
...
VOID CRenderSystem::Scene_SetLightmapQuality(INT nQuality)
{
	...
	{
	case 0:
		m_pRenderSystem->getTerrain()->setLightmapQuality(MM::Terrain::LMQ_NONE);
		m_pRenderSystem->getTerrain()->buildGeometry(m_pRenderSystem->getBaseSceneNode());
		break;
	...
	case 2:
		m_pRenderSystem->getTerrain()->setLightmapQuality(MM::Terrain::LMQ_HIGH);
		m_pRenderSystem->getTerrain()->buildGeometry(m_pRenderSystem->getBaseSceneNode());
		break;
...
VOID CRenderSystem::Scene_ShowGridSwitch(INT nZoneSize)
{
	...
	{
		m_pTerrainGrid = new MM::TerrainGrid(m_pRenderSystem->getSceneManager(), nZoneSize);
	}
...
{
	Ogre::Ray ray = m_pRenderSystem->getWindowToViewportRay(MM::Point(nX, nY));
	...
	for (Ogre::RaySceneQueryResult::const_iterator it = queryResult.begin(); it != queryResult.end(); ++it)
	{
		...
		MM::ObjectPtr object = MM::getObjectFromMovable(pMovable);
		if (object&&object->getData())
		{
            ...
            {
                MM::LogicModelObject* logicModelObject = static_cast<MM::LogicModelObject*>(object.get());

                assert (logicModelObject);

                MM::LogicModel* model = logicModelObject->getLogicModel();
...
{
//	MM::System::setAnimationQuality( (bSetting == TRUE)?1:0);
...
{
//	MM::PostFilterManager::setPostFilterEnabled("Floodlighting",(bSetting==TRUE));
