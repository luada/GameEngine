							...
							FLOAT fPitch)
{
	...
	FLOAT r_xz	= fDistance * ::cos(-fPitch);
	FLOAT x		= -r_xz * ::sin(fDirection);
	FLOAT z		= -r_xz * ::cos(fDirection);
...
FLOAT CCamera_Scene::_GetValidPitch(
							const MM::TerrainData* pTerrainData,	//地形数据
	...
	for(INT i=-1; i<=0; i++)
	{
		for(INT j=-1; j<=0; j++)
		{
			...
			if(pTerrainData->isValidGrid(nGridX, nGridZ))
			{
				...
				const MM::TerrainData::GridInfo& grid = pTerrainData->getGridInfo(nGridX, nGridZ);
