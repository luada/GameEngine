...
#include "Skill.h"
#include "AnimationRibbon.h"
#include "AnimationEffectInfo.h"
...
DObject_NT::DObject_NT( MM::System *system )
: m_pObjNode(NULL)
		...
		{			
			...
			{			
				...
				if ( mCurrentSkill )
				{
					...
					{
						MM::AnimationEffectInfo *effectInfo = mCurrentSkill->getAnimationEffectInfo(i);
						assert (effectInfo);
						...
						{
							MM::Effect *effect = effectInfo->getEffect();

							if (effect)
							{
								if ( !m_bAnimationLoop )
								{
									MM::EffectManager::getSingleton().removeEffect(effect, false);
									effect = NULL;
								}
							}
							else
							{
								effect = MM::EffectManager::getSingleton().createEffect(effectInfo->getEffectTemplateName());
					...
					{
						MM::AnimationRibbon *ribbon = mCurrentSkill->getAnimationRibbon(i);
						assert (ribbon);
					...
					if (m_fOnPlaySound)
					{
						...
						{					
							MM::AnimationSound *sound = mCurrentSkill->getAnimationSound(i);
							assert (sound);
...
bool DObject_NT::_LoadEntity()
{
	...
	for(it = m_EntityList.begin(); it != m_EntityList.end(); it ++)
	{
		try
		{
            ...
            Ogre::MeshPtr mesh = MM::loadMesh(it->File, m_GroupName, m_ObjFile, m_GroupName);
            if (!mesh.isNull())
            {
...
bool DObject_NT::_GetLocatorWorldTransform(const DString& name, MM::TransformInfo &info, const Ogre::Vector3 &offsetPos,
...
bool DObject_NT::_GetBoneWorldTransform(const DString& name, MM::TransformInfo &info, const Ogre::Vector3 &offsetPos,
										const Ogre::Quaternion &offsetRotation, GetTransformInfoType type )
{
...
void DObject_NT::CreateSkill(const DString &skillName, bool loop, bool anim, Ogre::Real delayTime)
{
	...
	if ( false == anim )
	{
		mCurrentSkill = MM::EffectManager::getSingleton().createSkill(skillName);
...
void DObject_NT::DelCurrentSkill(void)
{
	if (mCurrentSkill)
	{
		...
		MM::EffectManager::getSingleton().removeSkill(mCurrentSkill);
		mCurrentSkill = NULL;
	}
}
...
Ogre::Entity * DObject_NT::_LoadEntity( const DString& meshName )
{
	...
	try
	{
        ...
        Ogre::MeshPtr mesh = MM::loadMesh(meshName, m_GroupName, m_ObjFile, m_GroupName);
        if (!mesh.isNull())
        {
...
{	
	...
	MM::Effect *effect = MM::EffectManager::getSingleton().createEffect(effectName);
...
std::pair<bool, FairyEffectMap::iterator> DObject_NT::DelEffect( EffectHandle handle )
{
	...
	{
		MM::EffectManager::getSingleton().removeEffect((i->second).mEffect);
...
void DObject_NT::_updateFairyEffects(void)
{
	...
	{
		MM::Effect *effect = (i->second).mEffect;
