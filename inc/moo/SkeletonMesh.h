...
protected:	
	...
	MM::Skill *mCurrentSkill;

public:
	//构造函数
	DObject_NT( MM::System *system );
	...
	bool							_GetLocatorWorldTransform(const DString& name, MM::TransformInfo &info, const Ogre::Vector3 &offsetPos = Ogre::Vector3::ZERO,
		const Ogre::Quaternion &offsetRotation = Ogre::Quaternion::IDENTITY, GetTransformInfoType type = GTIT_ALL );
	bool 							_GetBoneWorldTransform(const DString& name, MM::TransformInfo &info, const Ogre::Vector3 &offsetPos = Ogre::Vector3::ZERO,
		const Ogre::Quaternion &offsetRotation = Ogre::Quaternion::IDENTITY, GetTransformInfoType type = GTIT_ALL );
