...
#if COMPILER == COMPILER_MSVC && (1300 <= COMP_VER && COMP_VER <= 1310)
...
#endif  // COMPILER == COMPILER_MSVC && COMP_VER == 1300
...
#else // #if defined(_WIN32__) || defined(_WIN32)
...
#endif // #if defined(_WIN32__) || defined(_WIN32)
...
#else   // COMPILER != COMPILER_MSVC

// Assumed wchar_t is natively for other compilers
#   define IS_NATIVE_WCHAR_T     1

#endif  // COMPILER == COMPILER_MSVC
	...
	*/
	...
	public:
		...
		public:
			...
			explicit invalid_data(const std::string& Message): std::runtime_error(Message)
			{
		...
		//@ExportToLua
		UString substr(size_type index, size_type num = ~(size_t)0) const
		{
		...
		void _cleanBuffer() const
		{
			if (m_buffer.mVoidBuffer != 0)
			{
				...
				default:
					...
					ASSERT(false, "This should never happen - mVoidBuffer should never contain something if we "
						"don't know the type");
					break;
				}
				m_buffer.mVoidBuffer = 0;
